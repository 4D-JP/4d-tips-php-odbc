...
PHPでODBCを使用する場合，ドライバーの文字コードは``system``に設定する必要があるようです。4Dから4DへのODBCであれば，``UTF-8``で通るので，ドライバーは``UTF-8``に対応しているといえそうですが，Windows版PHPのODBC拡張では``UTF-8``の受け渡しに問題があります。Open Query, MSAcces, Windevに加え，PHPの互換オプションも必要なのでしょうか・・・
